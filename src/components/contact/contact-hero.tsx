    ...
    <section className="pb-20 pt-16 sm:pb-28 sm:pt-24 lg:pb-36 lg:pt-32">
      <Container>
        <div className="flex flex-wrap items-center justify-between gap-3 font-mono text-[9px] tracking-[.12em] text-body" dir="ltr">
          <span>HB / CONTACT</span>
          <span>PROJECT / INQUIRY</span>
          <span className="flex items-center gap-2"><span className="size-1.5 bg-accent" />FORM: PREVIEW</span>
        </div>

        <div className="mt-12 grid gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,.34fr)] lg:items-end lg:gap-16">
          <div>
            <SectionLabel>شروع یک پروژه</SectionLabel>
            <h1 className="type-hero mt-8 max-w-4xl text-balance font-medium text-ink">
              بیایید درباره پروژه شما حرف بزنیم.
            </h1>
          </div>
          <div className="pb-2">
            <p className="text-pretty text-lg leading-9 text-body">
              لازم نیست همه‌چیز از ابتدا قطعی باشد. هدف، وضعیت فعلی و چند خط درباره مسئله، نقطه شروع خوبی برای یک گفت‌وگوی روشن است.
            </p>
            <p className="mt-7 font-mono text-[9px] tracking-[.12em] text-muted" dir="ltr">START SMALL / DEFINE TOGETHER</p>
          </div>
        </div>
      </Container>
    </section>
